...
class ProbabilitiesGenerator(nn.Module):
    def __init__(self, params, backbone, output_size):
        super().__init__()
        self.params = params
        self.backbone = backbone
        self.output_size = output_size  # Desired probability distribution output dimension
        self.ppad = PeriodicPad2d(1)
        self.conv = nn.Conv2d(
            backbone.out_chans, output_size, kernel_size=3, stride=1, padding=0, bias=True
        )
        # No activation function before softmax
        self.global_pool = nn.AdaptiveAvgPool2d(1)  # Global average pooling to reduce spatial dimensions

    def forward(self, x):
        x = self.backbone(x)  # Shape: (batch_size, out_chans, H, W)
        x = self.ppad(x)
        x = self.conv(x)  # Shape: (batch_size, output_size, H', W')
        x = self.global_pool(x)  # Shape: (batch_size, output_size, 1, 1)
        x = x.view(x.size(0), -1)  # Shape: (batch_size, output_size)
        x = F.softmax(x, dim=1)  # Apply softmax over the output dimension
